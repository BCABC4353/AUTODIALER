...
export function evaluate(
  ...
  now: Date,
  ctx: RuleContext,
  options: { consentZero?: boolean; bypassHours?: boolean; allowRepeats?: boolean } = {},
): DropReason | null {
  ...
  const repeats = Boolean(options.allowRepeats);
  const dayAgo = new Date(now.getTime() - MIN_HOURS_BETWEEN_ATTEMPTS * 3600 * 1000);
  if (!repeats && ctx.attemptsSince(patient.run, utcText(dayAgo)) > 0) return 'attempt_24h';
  const weekAgo = new Date(now.getTime() - ATTEMPT_WINDOW_DAYS * 86400 * 1000);
  if (!repeats && ctx.attemptsSince(patient.run, utcText(weekAgo)) >= MAX_ATTEMPTS_PER_WINDOW) return 'attempts_7d';
  const last = ctx.lastAttempt(patient.run);
  if (!repeats && last && last.outcome === HUMAN_OUTCOME && last.agent_note) return 'already_handled';
